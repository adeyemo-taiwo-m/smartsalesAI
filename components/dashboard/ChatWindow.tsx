...
export function ChatWindow({ className = "" }: ChatWindowProps) {
  ...
  if (!selectedLead) {
    return (
      <div className={cn("bg-dark-card rounded-xl border border-slate-700/50 shadow-card flex items-center justify-center text-center p-8 h-[540px]", className)}>
        <div className="space-y-3">
          <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 mx-auto">
            <Sparkles size={22} className="animate-pulse" />
          </div>
          <h4 className="text-sm font-semibold text-slate-200">No Chat Selected</h4>
  ...
  return (
    <div className={cn("bg-dark-card rounded-xl border border-slate-700/50 shadow-card flex flex-col justify-between overflow-hidden h-[540px] group hover:border-slate-600 transition-colors", className)}>
      {/* Header Info */}
      <div className="px-5 py-3 border-b border-slate-700/50 flex items-center justify-between bg-dark/15 select-none">
      ...
      >
        {messages.length > 0 ? (
            ...
            return (
              ...
              >
                <div className="max-w-[75%] space-y-1">
                  {/* Sender title tags */}
                  {!isCustomer && (
                    <div className={cn("flex items-center gap-1.5 text-[9px] mb-0.5", isAi ? "justify-end text-purple-400 font-bold" : "justify-end text-slate-400 font-bold")}>
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        isCustomer ? "text-slate-500" : isAi ? "text-blue-200" : "text-slate-300"
                      )}
                    >
          ...
          <div className="h-full flex items-center justify-center text-center p-6">
            <div>
              <p className="text-xs text-slate-500">No messages in this chat pipeline.</p>
      ...
      <div className="border-t border-slate-700/50 p-4 bg-dark-card space-y-2 select-none">
        ...
        <form onSubmit={handleSend} className="flex items-center gap-2">
          <button type="button" className="text-slate-500 hover:text-slate-300 transition-colors shrink-0">
            <Smile size={16} />
          </button>
          <Input
            value={inputValue}
            ...
            className="flex-1 h-9 text-xs bg-dark border-slate-800 text-slate-200 placeholder:text-slate-600 rounded-full focus-visible:ring-1 focus-visible:ring-blue-500"
          />
          <button type="button" className="text-slate-500 hover:text-slate-300 transition-colors shrink-0">
            <Paperclip size={16} />
          </button>
