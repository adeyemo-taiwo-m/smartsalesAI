...
**Version:** 1.1 | **Stack:** React + Next.js 14 (App Router) + Tailwind CSS + shadcn/ui | **Theme:** Dark Only
...
| Token Name | Hex | Tailwind Class | Used For |
|---|---|---|---|
| `brand-green` | `#1D6B4A` | `bg-brand-green` / `text-brand-green` | Primary CTAs, active nav, links, primary buttons, focus rings |
| `brand-purple` | `#7C3AED` | `bg-brand-purple` / `text-brand-purple` | AI indicators, secondary accents, gradient pair with green |
| `brand-green-light` | `#22C55E` | `bg-brand-green-light` / `text-brand-green-light` | Success states, completed sales, WhatsApp channel, positive trends |
...
| Info | `#1D9E75` | `text-brand-green` / `bg-brand-green` | New lead status, informational banners |
...
export const CHANNEL_COLORS = {
  ...
  web:       { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', hex: '#1D9E75' },
};
```
...
export const LEAD_STATUS_COLORS = {
  new:    { bg: 'bg-emerald-500/10',  text: 'text-emerald-400',  border: 'border-emerald-500/20' },
...
The primary logo and hero headline use a green-to-purple gradient:
...
| Card hover | `border-brand-green/50` (transition on hover) |
| Sidebar active nav item | `bg-brand-green/10 text-brand-green` (no left-border; full bg highlight instead) |
| Input default | `border border-slate-700` |
| Input focus | `border-brand-green/60 ring-2 ring-brand-green/15` |
...
```
new / info:     border border-emerald-500/20
...
className="border-2 border-brand-green shadow-lg shadow-brand-green/20"
```
...
| `shadow-green` | Primary button hover; highlighted pricing card |
| `shadow-purple` | AI message bubbles, AI indicator glow |
| `shadow-green-soft` | Success confirmation banners |
...
<div className="bg-dark-card rounded-xl border border-slate-700/50 shadow-card
                hover:border-brand-green/30 hover:shadow-card-hover
...
- Hover: `border-brand-green/30`, `shadow-card-hover`, subtle `translateY(-2px)` (via framer-motion or `hover:-translate-y-0.5`)
- Never add a colored left border to standard cards
- Pricing highlighted card only: `border-2 border-brand-green shadow-pricing-glow`
...
```tsx
// Primary (main CTA — forest green)
"bg-brand-green hover:bg-brand-green/80 text-white font-semibold rounded-lg
 shadow-green hover:-translate-y-0.5
 focus:outline-none focus:ring-2 focus:ring-brand-green/50
 transition-all duration-200
 disabled:opacity-40 disabled:cursor-not-allowed disabled:translate-y-0"

// Primary gradient (landing CTAs only)
"bg-gradient-to-r from-brand-green to-purple-600
 hover:from-brand-green/80 hover:to-purple-500
 text-white font-semibold rounded-full
 shadow-green hover:-translate-y-0.5
 transition-all duration-200"

// Secondary / outline
"bg-transparent border border-slate-600 hover:border-brand-green/50
...
// Chats:   bg-brand-green/10   icon: text-brand-green
...
<div
  ...
  className={cn(
    ...
    isSelected
      ? "bg-brand-green/10 border-l-2 border-l-brand-green"
...
- Selected state: `bg-brand-green/10` + `border-l-2 border-l-brand-green`
...
<div className="flex justify-end mb-3">
  <div className="max-w-[75%]">
    <div className="flex items-center justify-end gap-1.5 mb-1">
      ...
      {intentTag && (
        <span className="bg-brand-green/10 text-brand-green border border-brand-green/20 text-xs px-1.5 py-0.5 rounded-full">
          {intentTag}
        </span>
      )}
    </div>
    <div className="bg-gradient-to-br from-brand-green to-brand-green/70 text-white rounded-lg rounded-tr-sm px-3.5 py-2.5 text-sm shadow-green">
      {content}
      <span className="block text-xs text-green-100/70 mt-1 text-right">{time}</span>
    </div>
  </div>
</div>
...
const AVATAR_COLORS = [
  '#1D6B4A', '#7C3AED', '#059669', '#D97706',
  '#DC2626', '#0F6E56', '#7C3AED', '#BE185D',
];
...
<Toaster
  ...
  toastOptions={{
    ...
    success: {
      style: { borderLeft: '3px solid #1D6B4A' },
      iconTheme: { primary: '#1D6B4A', secondary: '#1E293B' },
    },
    error: {
...
<div className="bg-dark-card rounded-2xl border-2 border-brand-green shadow-pricing-glow
                p-6 flex flex-col relative scale-105">
  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
    <span className="bg-gradient-to-r from-brand-green to-purple-600 text-white text-xs font-semibold
...
// Floating bubble
<button
  className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-gradient-to-br from-brand-green to-purple-600
...
<div className="flex flex-col items-center justify-center py-16 text-center">
  ...
  {cta && (
    <button className="mt-4 px-4 py-2 bg-brand-green hover:bg-brand-green/80 text-white text-sm font-semibold rounded-lg transition-colors">
      {cta}
    </button>
  )}
</div>
...
|---|---|---|---|
| Card | `border-slate-700/50` | `border-brand-green/30` + `translateY(-2px)` | — |
| Nav item | `text-slate-400`, transparent | `text-slate-50 bg-slate-700/50` | `bg-brand-green/10 text-brand-green` |
| Input | `border-slate-700` | same (no change) | `border-brand-green/60 ring-2 ring-brand-green/15` |
| Button primary | `bg-brand-green` | `bg-brand-green/80 translateY(-1px)` | `ring-2 ring-brand-green/50` |
| Button secondary | `border-slate-600` | `border-brand-green/50 text-slate-50` | `ring-2 ring-brand-green/50` |
| Conversation item | transparent | `bg-slate-700/30` | `bg-brand-green/10 border-l-2 border-l-brand-green` |
| Table row | transparent | `bg-slate-700/20` | — |
| Tab / filter | `text-slate-400` | `text-slate-200` | `bg-dark-card text-slate-50` |
| Sidebar item | `text-slate-400` | `text-slate-50 bg-slate-700/50` | `bg-brand-green/10 text-brand-green` |
...
<defs>
  <linearGradient id="greenGradient" x1="0" y1="0" x2="0" y2="1">
    <stop offset="5%"  stopColor="#1D6B4A" stopOpacity={0.3} />
...
```ts
const PIE_COLORS = ['#1D6B4A', '#EC4899', '#22C55E'];
// whatsapp → forest green, instagram → pink, web → lime green
```
...
<section
  ...
  style={{
    background: `
      radial-gradient(ellipse 80% 50% at 50% -10%, rgba(29,107,74,0.25) 0%, transparent 70%),
      #0F172A
    `
  }}
>
...
```tsx
// Full-width green-to-purple gradient section
<section className="bg-gradient-to-r from-brand-green to-purple-700 py-20">
```
...
const channelBadges = [
  ...
  { label: 'Website Chat', color: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'   },
];
```
...
<div className="bg-dark-card rounded-2xl border border-slate-700/50 shadow-card p-6
                hover:border-brand-green/30 hover:shadow-card-hover hover:-translate-y-1
...
_SmartSales AI Style Guide — v1.1_
